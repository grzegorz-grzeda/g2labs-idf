...
static void load_predefined_file(void) {
    size_t file_size = g2l_os_fs_get_size_of_file("test2.txt");
    printf("File size: %d\n", file_size);
    char buffer[256];
    size_t read_size = g2l_os_fs_load_file("test2.txt", buffer, sizeof(buffer));
    printf("Read size: %d\n", read_size);
    printf("Read data: '%.*s'\n", read_size, buffer);
}
